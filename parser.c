  ...
  while(lex != EOFILE)
  {
    switch(lex)
    {
        ...
        case OPEPAR:
            npar++;
        break;
        case CLOPAR:        // zamykający nawias - to może być koniec prototypu, nagłówka lub wywołania
        {
            ...
            {         // sprawdzamy, czy liczba nawiasów bilansuje się z wierzchołkiem stosu funkcji
                ...
                if(nlex == OPEBRA)
                {   // nast. leksem to klamra a więc mamy do czynienia z def. funkcji
                    store_add_def(get_from_fun_stack(), ln_nr, inpname);
                }
                else if(nbra == 0)
                {   // nast. leksem to nie { i jesteśmy poza blokami - to musi być prototyp
...
int comp(const void *aa, const void *bb)
{
    ...
    if(c != 0)				            // Porównanie po nazwie pliku
	    return c;
    if(a->numer_linii > b->numer_linii)	// Porównianie po numerze linii
        return 1;
    else if(a->numer_linii < b->numer_linii)
        return -1;
    return 0;
}
...
int wypisywacz(void)
{
    ...
    tmp_numer_linii = (Kont->kont[0]).numer_linii;

    printf("\n\nFunkcja '%s'\n\t%s:\n\t\t%s w linijce %d\n", tmp_nazwa, typ(tmp_typ), tmp_plik, tmp_numer_linii);

    for(int i = 1; i < petl; i++)
    {
	    temp = Kont->kont[i];
	    tmp_numer_linii = temp.numer_linii;
